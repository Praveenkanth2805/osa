...
export async function GET(req: NextRequest) {
  ...
  const worksheetData = students.map(s => ({ 'Register Number': s.registerNumber, 'Name': s.name, 'Gender': s.gender, 'Department': s.department?.name, 'Course': s.course?.name, 'Fee Amount': s.course?.fee }))
